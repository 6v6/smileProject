...
#TRAINING_FILE = 'gs://'+DESTINATION_BUCKET+'/'+"training_happiness.txt"
#VALIDATION_FILE = 'gs://'+DESTINATION_BUCKET+'/'+"training_happiness.txt"
...
def main(argv=None):
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=True)) as sess:
        ...
        #base_model_dir = os.path.join('FaceEmotion', 'model')
        #if not os.path.exists(base_model_dir):
         #   os.makedirs(base_model_dir)

        saver.save(sess,'gs://emotion-training-data/model5/emotion_recog')
        #saver.save(sess, '/FaceEmotion/emotion_recog') 
        #gcs_copy(base_model_dir, 'gs://emo-training'+'/export')
        #base 에서 버킷으로복사?
        coord.request_stop()
        coord.join(threads)
        print('finish')
