...
DESTINATION_BUCKET = 'emotion-training-data'
#TRAINING_FILE = 'gs://'+DESTINATION_BUCKET+'/'+training_file3.txt
#VALIDATION_FILE = 'gs://'+DESTINATION_BUCKET+'/'+validate_file3.txt
TRAINING_FILE = "training_emotion.txt"
VALIDATION_FILE = "validate_emotion.txt"
...
flags.DEFINE_string("log_dir", "gs://emotion-training-data/", "log_dir")
#FLAGS.base_dir='/tmp/'
flags.DEFINE_string("base_dir","FaceEmotion/","base_dir")
#FLAGS.MAX_TRAINING_STEP = 15000
flags.DEFINE_integer("MAX_TRAINING_STEP",15000,"MAX_TRAINING_STEP")
#FLAGS.local_image_dir=FLAGS.base_dir+'images/'
flags.DEFINE_string("local_image_dir",'FaceEmotion/images/',"local_image_dir")
...
def main(argv=None):
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=True)) as sess:
        ...
        print('Save model')
        model_dir = os.path.join( 'gs://emotion-training-data' , 'model5')
        if not os.path.exists(model_dir):
            os.makedirs(model_dir)
        saver.save(sess, os.path.join(model_dir, 'emotion_recog'))
        print('Save model done '+model_dir)

        export_dir = os.path.join( 'gs://emotion-training-data' , 'export_model5')
        print('export path : '+export_dir)
        if  os.path.exists(export_dir):
            rmdir(export_dir)
        export_model(os.path.join(model_dir, 'emotion_recog'), export_dir)
        print('export model done '+export_dir)
        
        
        gcs_copy(model_dir, 'gs://emotion-training-data/')
        gcs_copy(export_dir, 'gs://emotion-training-data/')
